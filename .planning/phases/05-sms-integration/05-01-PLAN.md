...
<objective>
Implement Twilio SMS webhook to parse "amount category note" messages and create transactions automatically.

Purpose: Enable zero-friction expense logging via text message - the core value proposition of the app.
Output: Working SMS webhook that creates transactions from texts like "$25 coffee lunch with team".
</objective>
...
- Phase 04-02: createTransaction Server Action exists, takes amount, category_id, note, transaction_date, budget_id, user_id, source
...
<task type="auto">
  <name>Task 2: Implement SMS text parser and category matching</name>
  <files>lib/twilio/parser.ts, app/api/sms/route.ts</files>
  <action>
Create lib/twilio/parser.ts with parseSMS function that extracts amount, category, note from SMS body.
...
2. Remove amount from text, split remaining by whitespace
3. First word after amount = category string (or null if none)
4. Remaining words = note (or null if none)

Return interface: `{ amount: number | null, category: string | null, note: string | null }`
...
Then create lib/twilio/categories.ts with matchCategoryToId function:
- Query categories table for current user (TEMP_USER_ID)
- Case-insensitive exact match on category string
- Return category_id or null if no match (will create uncategorized transaction)

From RESEARCH.md: Be permissive with parsing. Accept multiple amount formats. Default missing fields to null rather than rejecting. Don't use NLP libraries - regex is sufficient for controlled "amount category note" format.
  </action>
  <verify>Unit test parseSMS with various inputs, verify parsing handles edge cases ($25, 25, 25.5, "25 coffee", "25.50 groceries trader joes")</verify>
  <done>Parser extracts amount/category/note correctly, handles edge cases (missing category, missing note, various amount formats), category matcher queries database case-insensitively</done>
</task>
...
2. Parse SMS body using parseSMS(Body)
   - If amount is null, return TwiML error message: `<Response><Message>Could not parse amount. Format: "$25 coffee lunch"</Message></Response>`

3. Match category string to category_id using matchCategoryToId(category, TEMP_USER_ID)
   - Returns null if no match (that's okay - creates uncategorized transaction)

4. Determine budget_id:
...
5. Create transaction using existing createTransaction pattern (NOT the Server Action - direct Supabase insert):
   - Insert into transactions: amount, category_id (nullable), note (nullable), transaction_date (now), budget_id, user_id (TEMP_USER_ID), source: 'sms', twilio_message_id: MessageSid, twilio_from: From

6. Use revalidatePath(`/finance/budgets/${budget_id}`) to refresh UI

7. Return success TwiML: `<Response></Response>` (empty = silent success, no SMS reply)
...
From RESEARCH.md: Respond within 15 seconds (Twilio timeout). Check MessageSid for idempotency before any processing. Twilio retries on timeout, causing duplicate transactions if not handled. Use database insert directly in webhook (Server Actions are for client-side forms, webhooks need raw speed).
  ...
  <verify>Test with curl POST to /api/sms with Twilio-formatted body, verify transaction created, verify duplicate MessageSid doesn't create duplicate transaction</verify>
  <done>Webhook creates transactions from SMS, idempotency prevents duplicates, category matching works, budget assignment automatic, responds within 15 seconds, UI refreshes after transaction creation</done>
</task>
...
5. Test SMS parsing:
   - Text your Twilio number: "$25 coffee"
   - Visit: http://localhost:3000/finance/budgets
   - Click on current budget
   - Verify: Transaction appears with amount $25, category "coffee" (if category exists) or "Uncategorized"
...
6. Test various formats:
   ...
   - Text: "100"
   - Text: "$5 snack"
   - Verify all create transactions with correct parsing
...
- [ ] Text parsing extracts amount, category, note from various formats
- [ ] Transactions created with source='sms' and twilio_message_id
- [ ] Idempotency prevents duplicate transactions
- [ ] Category matching works case-insensitively
